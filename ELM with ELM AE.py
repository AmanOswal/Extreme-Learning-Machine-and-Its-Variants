...
class Gen_ELM2(object):
    def __init__(self,sess,batch_size,input_len,hidden_lens,output_len,elmop):
    ...
    def test(self, x, t=None):
        ...
        if t is not None :
            print("Accuracy of ELM with ELM AutoEcoder {:.9f}".format(self.sess.run(self.accuracy, {self.x_test:x, self.y_test:t})))
        else :
...
elm = Gen_ELM2(sess, batch_size, 784, hidden_num, 10,elmop)
